...
document.write(
  `<table style="border: 1px solid black; border-collapse: collapse;  margin: auto"><tr><th style="padding: 8px 15px;">Name</th><th style="padding: 8px 15px;">Cuisine</th></tr>`
);
let arr = data.recipes.flatMap((x) => {
  let flag = false;
  x.tags.flatMap((y) => {
    if (y == "Indian" || y == "Italian") flag = true;
  });
  if (flag == true) {
    document.write(
      `<tr><td style="border: 1px solid black; padding: 8px 15px;">${x.name}</td><td style="border: 1px solid black; padding: 8px 15px;">${x.cuisine}</td></tr>`
    );
